...
<template>
  <div class="h-screen flex-1 flex flex-col relative">
    ...
    <div id="mainContent" class="flex items-start justify-center">
      <Loader :size="90" v-if="fetchingRecords || !records || !selectedModel || !selectedModel.fields" />
      <div
        v-else
        class="px-8 pt-2 pb-8 border-red border-2 rounded-md"
        style="width:100%; max-width: 800px;"
      >
        <TableRow :fields="selectedModel.fields" :data="records[0]" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(["records", "fetchingRecords", "selectedModelName", "selectedRecord"]),
